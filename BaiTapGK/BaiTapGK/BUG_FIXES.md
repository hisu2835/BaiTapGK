...
```csharp
// BEFORE: Có th? crash khi accessing null objects
...
```csharp
// BEFORE: Crash n?u không có Segoe UI Emoji
...
### 4. **Memory Leaks**
```csharp
// BEFORE: Timer không ???c dispose properly
animationTimer.Stop();
...
```csharp
// BEFORE: Animation có th? ch?y multiple instances
...
### 6. **Paint Exceptions**
```csharp
// BEFORE: Crash n?u Graphics operations fail
g.DrawString(displayGesture, gestureFont, mainBrush, x, y);

// AFTER: Try-catch v?i fallback rendering
try {
    g.DrawString(displayGesture, gestureFont, mainBrush, x, y);
} catch {
    // Fallback rendering
...
```csharp
// BEFORE: Crash n?u sound system unavailable
...
```csharp
// BEFORE: Crash v?i empty/null choices
finalChoice = choice;
